...
pub struct NanonisConnection {
    ...
    stamp_rx: std::sync::mpsc::Receiver<StaticImage>,
    frame_rx: std::sync::mpsc::Receiver<FrameDataGrabResponse>,
...
impl NanonisConnection {
    ...
    pub fn update(&mut self, encoder: &ImageEncoder) -> Vec<StaticImage> {
        ...
        while let Ok(mut image) = self.stamp_rx.try_recv() {
...
async fn scan_worker(
    ...
    loop {
        ...
        if *scanning.borrow() && channel_rx.borrow().is_some() {
            ...
            let frame_info = conn.scan_frame_get().await.unwrap();
            let transform = transform_from_frame(&frame_info);
            let props = conn.scan_props_get().await.unwrap();
            let name = format!("{}({})", props.series_name, stamp_num);
            stamp_num += 1;
            let mut image = StaticImage::new(&encoder, size, transform, |buf| {
                buf.copy_from_slice(&frame.scan_data.data)
            });
            image.channel = Some(channel as usize);
            image.name = name;
            image.signal_names = signal_names_rx.borrow().clone();
            image.channels = vec![channel as usize];
...
async fn line_worker(
    ...
    ctx: egui::Context,
    scanning: watch::Sender<bool>,
    frame_tx: std::sync::mpsc::Sender<FrameDataGrabResponse>,
    channel_rx: watch::Receiver<Option<usize>>,
) {
    let mut conn = nonblocking::NanonisTcp::new(addr).await.unwrap();
    loop {
