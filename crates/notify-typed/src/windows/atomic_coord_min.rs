...
use bbq2::traits::coordination::{Coord, ReadGrantError, WriteGrantError};

/// Coordination using CAS atomics
pub struct AtomicCoordMin {
...
impl Default for AtomicCoordMin {
    fn default() -> Self {
        Self::new()
    }
}

unsafe impl Coord for AtomicCoordMin {
    ...
    fn grant_max_remaining(
        ...
        let start = if already_inverted {
            ...
            if (max - write) >= sz {
                // All room remaining in un-inverted case
                sz = max - write;
                write
            } else {
                // Not inverted, but need to go inverted

                // NOTE: We check read > sz, NOT read >= sz, because
